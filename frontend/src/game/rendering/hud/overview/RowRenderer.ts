...
export class PlayerRowRenderer {
    ...
    private drawBackground(player: PlayerOverviewEntry, bounds: Rect) {
        ...
        if (player.isCurrentTurn) {
            ctx.fillStyle = theme.backgroundActive;
        } else if (player.isLocalPlayer) {
            ctx.fillStyle = theme.backgroundLocal;
        } else {
        ...
        // Subtle outline for the local player (unless it's already highlighted for the turn)
        if (player.isLocalPlayer && !player.isCurrentTurn) {
    ...
    private drawName(player: PlayerOverviewEntry, bounds: Rect) {
        ...
        ctx.fillText(player.name, nameX, nameY, 60);
    ...
    private drawStats(player: PlayerOverviewEntry, bounds: Rect) {
        ...
        ctx.fillStyle = player.resCardCount > 0
            ? theme.statColor
            : theme.statMutedColor;
        ctx.fillText(`${player.resCardCount}🂠`, cardX, statsY);
