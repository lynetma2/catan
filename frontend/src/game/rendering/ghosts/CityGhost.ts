...
export type CityGhostVariant = 'indicator' | 'preview';

export class CityGhost implements GhostEffect {
    private readonly vertex: Vertex;
    private readonly color: string;
    private readonly variant: CityGhostVariant;

    constructor(vertex: Vertex, color: string = "rgba(255, 255, 255, 0.5)", variant: CityGhostVariant = 'indicator') {
        this.vertex = vertex;
        this.color = color;
    ...
    draw(ctx: CanvasRenderingContext2D, layoutSettings: LayoutSettings): void {
        if (this.variant === 'preview') {
            BuildingRender.drawGhostPreview(ctx, layoutSettings, this.vertex, this.color, BuildingType.City);
        } else {
