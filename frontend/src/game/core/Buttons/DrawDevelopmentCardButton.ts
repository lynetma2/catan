...
import {DevelopmentCardBackSVGString} from "@/assets/assets.tsx";
import type {InteractionManager} from "@/game/input/InteractionManager.ts";

export class DrawDevelopmentCardButton implements Button {
    type: ButtonType;
    ...
    constructor(bounds: {
        ...
        width: number;
        height: number
    }, canvas: HTMLCanvasElement, color: string, id: string, manager: InteractionManager) {
    ...
    draw() {
        ...
        image.src = DevelopmentCardBackSVGString(this.iconWidth, this.iconHeight, this.color);
