...
@Component
@HandlesEvent(EndSummaryAction.class)
public class EndSummaryHandler implements GameActionHandler<EndSummaryAction> {
    private final GameStore gameStore;
    private final GameMapper gameMapper;

    public EndSummaryHandler(GameStore gameStore, GameMapper gameMapper) {
        this.gameStore = gameStore;
        this.gameMapper = gameMapper;
    }

    @Override
    public EventResult<ServerEvent> handle(GameContext context, EndSummaryAction action) {
        Game game = gameStore.get(context.gameId());

        return EventResult.directed(context.playerId(), new EndSummaryEvent(gameMapper.toEndSummaryDTO(game)));
    }
}
